...
- **Voice**: MediaRecorder → auto-detected MIME (webm/mp4/ogg) → **preview with FX** → offline render → presigned URL upload → message with `mediaType: "audio"`. After recording, members enter a preview step where they can play back their recording through the FX chain (speed, crush, delay, feedback, presets), add an optional caption, then send or discard. When FX are active (any parameter differs from clean), the audio is rendered offline through the same effects chain via `OfflineAudioContext`, encoded as WAV, and uploaded — so other members hear the processed audio. If no FX are applied, the raw recording is uploaded as-is. Fallback: if offline render fails, raw audio is sent.
