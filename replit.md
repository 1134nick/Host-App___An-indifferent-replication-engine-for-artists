...
- **Web Audio API engine**: Full FX bus: source → inputGain → highpass → compressor → [dry/wet split] → toneFilter → waveshaper → delay (with feedbackFilter loop) → limiter → outputGain → analyser → destination. Proper gain staging prevents clipping.
- **FX panel**: Expandable effects rack with nine controls: INPUT (0.5–2.0), SPEED (0.25x–2x), HPASS (40–180Hz), TONE (500–8000Hz), CRUSH (0–100), DELAY (0–1.2s), FEEDBACK (0–75%), MIX (0–100% wet/dry), OUTPUT (0.2–1.2). Six presets: CLEAN, HAUNTED, CRUSHED, SUBMERGED, VOID, NERVE — remapped around the full bus for distinct tonal character.
- **Non-destructive FX**: Raw audio is always uploaded. FX settings are stored in `mediaMeta.fx` JSON on the message record. Playback applies stored FX live via Web Audio API, meaning older messages can be reinterpreted with different settings.
