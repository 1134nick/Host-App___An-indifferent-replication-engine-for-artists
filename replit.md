...
- **FX panel**: Expandable effects rack organized into sections: Gain (SPEED/INPUT/OUTPUT), EQ (HPASS/TONE/LO SHELF/HI SHELF), Modulation (type selector + RATE/DEPTH), Amp (MIX/CRUSH), Delay (type selector + DELAY/DELAY R/FEEDBACK), Vocoder (ON/OFF + FORMANT). Two preset banks:
  ...
  - **Mathematical constant presets**: 0 (nullity — near-silence, minimum gain/speed/EQ, maximally reduced signal), 1 (unity — pure identity pass-through), 2 (octave — 2x speed, doubled frequencies, stereo chorus), 3 (triad — 1.5x speed for perfect fifth, 3-voice ensemble, 1/3 ratios throughout), π (cycle — phaser at 3.14Hz, delay/feedback/mix all π-derived, tone at 3142Hz), φ (golden spiral — 0.618x speed, φ/1-φ delay ratio in L/R, Fibonacci-sequence frequency 2584Hz), √2 (equal temperament — 1.414x speed for semitone shift, flanger at √2 rate, 0.707 depth), e (natural decay — 0.718x speed, 1/e mix, feedback at 1-1/e, organic chorus), i (imaginary — 90° phase rotation via full-depth phaser, equal L/R delays, 50% wet/dry split), ℙ (primes — prime-derived parameters: 97Hz highpass, 23/37/53/59 mapped across controls, vocoder enabled with +5 formant shift).
