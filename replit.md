...
- **FX panel**: Expandable effects rack organized into sections: Gain (SPEED/INPUT/OUTPUT), EQ (HPASS/TONE/LO SHELF/HI SHELF), Modulation (type selector + RATE/DEPTH), Amp (MIX/CRUSH), Delay (type selector + DELAY/DELAY R/FEEDBACK), Vocoder (ON/OFF + FORMANT). Eight presets: CLEAN, HAUNTED, CRUSHED, SUBMERGED, VOID, NERVE, ROBOT, CATHEDRAL — each configured with full FX chain parameters.
- **RAW/BAKED upload toggle**: Before sending, members choose RAW (default — upload raw audio, apply FX live on playback) or BAKED (render all FX into the audio file via `renderWithFx` offline renderer, upload as WAV). Baked messages store `mediaMeta.fx.baked = true` and play back with clean/neutral FX (no double-processing). When BAKED is selected but no FX are active, the UI indicates "no fx to bake" and sends raw. Offline rendering uses `OfflineAudioContext` with tail duration accounting for the maximum of left/right delay times.
